...
const AccommodationCard = ({ accommodation }: AccommodationCardProps) => {
  const { id, name, type, shortDescription, price, guests, beds, images } = accommodation;
  ...
  return (
    <Card className="overflow-hidden transition-all hover:shadow-lg">
      ...
      <CardContent className="p-5">
        <h3 className="text-xl font-heading font-semibold text-forest-dark mb-0">{name}</h3>
        {id === "wooden-house" ? (
          <p className="text-sm text-forest mb-3">Metaxaki</p>
        ) : (
          <p className="text-sm text-forest mb-3">Metaxoula</p>
        )}
      ...
      <CardFooter className="px-5 py-4 border-t flex justify-between items-center">
        ...
        >
          <Button className="bg-forest hover:bg-forest-dark">View Details</Button>
        </Link>
      </CardFooter>
    </Card>
  );
