...
/**
 * The two properties, as shown on the site.
 *
 * These used to live in a database, but they change once or twice a year (mostly
 * seasonal prices) and every visitor paid a network round-trip for them. Keeping
 * them here renders instantly and means the site has no runtime dependency.
 *
 * Guest-facing names and descriptions come from the translation files; the
 * English strings below are the fallback. Live availability is separate — see
 * api/availability.ts.
 *
 * The two glamping tents are identical units presented as one listing; which
 * physical tent a guest gets is decided when the booking is confirmed.
 */
